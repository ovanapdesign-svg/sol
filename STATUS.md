...
| Phase 4.3 dalis 2 — Product Builder Simple Mode UI                        | complete | The visible piece on top of the dalis 1 orchestrator. The Woo product ConfigKit tab now mounts a single-page Product Builder above the existing 8-section advanced view. Owners flip between the two with a "Show advanced settings" toggle in the header — same DOM, just visibility. **Header bar** carries the page title, a status pill (Disabled / Setup in progress / Ready / Live, colour-coded), and the Advanced toggle. **Block 1 — Product type**: grid of cards (Markise / Screen / Pergola / Terrassetak / Custom) with inline lucide-style SVG icons; click → POST `/product-builder/{id}/product-type` → snapshot reloaded → blocks 2-10 appear filtered by the recipe. **Block 2 — Pricing**: Manual rows tab (from-w / to-w / from-h / to-h / price grid with ✕ remove + Add row + Save) + Import Excel tab (drop zone hits `/imports` with `target_lookup_table_key` already wired to the auto-managed `lookup_table_key`, auto-commits on success). **Block 3 — Fabrics**: card per fabric with image (wp.media picker), Name / Code / Collection / Color family / Price group / Extra price / Active / Delete + Excel import wired to the auto-managed `fabric_library_key`. **Block 4 — Profile colors** (markise / pergola / terrassetak): card per color with Image / Name / Code / Hex (live swatch preview) / Active / Delete. **Block 5 — Operation mode**: three radio cards (Manual only / Motorized only / Both) → POST `/operation-mode` → blocks 6 + 7 visibility recomputed. **Block 6 — Stang options**: card with image / name / code / length cm / price / active. **Block 7 — Motor options**: two tabs — Single (image, name, code, Woo product picker, price source radio Use Woo / Use custom, custom price field, active) and Bundle (package name, code, image, components rows with Woo picker + qty + ✕ + + Add component, price strategy radio Sum of components / Custom package price, fixed package price field, active). **Blocks 8 + 9 — Controls + Accessories**: identical card shape factored into one renderer (image, name, code, optional Woo product picker, price source, custom price, active). **Block 10 — Ready-to-sell checklist**: one row per check from `can_enable_configurator()`, green ✓ on done, red ✗ on blocking, "optional" pill on non-required entries. Enable button unlocks only when ready; click → POST `/enable` → orchestrator writes `_configkit_enabled = 1` + template_key + lookup_table_key to product post meta → status pill turns green. **Cursor-jump fix from real-data testing honoured throughout**: input values are read from the DOM at structural-change time (add row, remove row, save) — render() never overwrites a value the owner is in the middle of typing. Drafts in `state.drafts` populate on first entry and on remove/add operations only. **WooProductPicker reuse**: every Woo-linked field (motor single, motor bundle components, controls, accessories) instantiates the existing `ConfigKit.createWooProductPicker` from Phase 4.2b.2 with object-identity-stable drafts so picker selections survive re-renders. **Friendly errors**: every save surfaces the orchestrator's owner-friendly message verbatim — never leaks `template_key` / `library_key`. **Status flip**: `ProductBuilderService::get_full_snapshot` now reads `_configkit_enabled` from post meta and surfaces it on `state.enabled` so the JS doesn't need a second round-trip after Enable to know whether the product is live. **`/snapshot` endpoint** aggregates the eight per-block reads into one round-trip (state, pricing_rows, fabrics, profile_colors, stangs, motors, controls, accessories, checklist) so the page renders pre-filled on open. Tests: 2 new ProductBuilderServiceTest cases (full snapshot aggregates, empty product). Suite 563 / 1518 → 565 / 1531; engine purity grep at zero. Promotes the Phase 4.3 status from `partial` to `complete`. |
| Phase 4.3 — Product Builder Simple Mode (orchestration layer)             | complete | Owner-facing orchestrator that translates "I have a Markise" into the underlying Module / Library / Library item / Lookup table / Template plumbing. Reuses every existing service — no new entities, no DB schema changes. **`ProductTypeRecipes`** ships five product types (Markise, Screen, Pergola, Terrassetak, Custom); each declares its family + the blocks it renders (pricing, fabrics, profile_colors, operation, stang, motor, controls, accessories). Adding a new product type = editing one PHP map. **`ProductBuilderState`** wraps `_configkit_pb_meta` post meta (product_type, template_key, family_key, lookup_table_key, fabric / color / stang / motor / control / accessory library keys) so the orchestrator stays idempotent across re-saves. **`AutoManagedRegistry`** is a single WordPress option recording every entity Simple Mode creates ("module:textiles", "library:product_42_fabrics", "lookup_table:product_42_pricing", "template:product_42_markise"); Advanced admin lists query it via REST and render a 🔧 Auto-managed badge next to the technical key with a tooltip warning that manual edits may be overwritten on the next save. **`ProductBuilderService`** orchestrator methods: `set_product_type` (creates family + draft template), `save_pricing_rows` (mints `product_{id}_pricing` lookup table with unit=mm, match_mode=round_up; replaces all cells on save), `save_fabrics` (provisions Textiles module via ModuleTypePresets defaults + per-product fabric library + items), `save_profile_colors` / `save_stangs` / `save_motors` / `save_controls` / `save_accessories` (one shared `save_role_items` pipeline; capability-aware payload mapping; motor block accepts `components` for Phase 4.2b bundles with fixed_bundle pricing), `save_operation_mode` (records manual_only / motorized_only / both), `can_enable_configurator` (recipe-driven readiness checklist with id / label / done / required), `enable_configurator` (refuses when not ready; flips `_configkit_enabled` + writes `_configkit_template_key` + `_configkit_lookup_table_key` so the storefront renderer picks up the auto-managed entities). **`ModuleService::parse_schema`** now accepts both legacy `{key:type-string}` and rich `{key:{label,type,...}}` shapes so apply_to_payload's preset attribute schemas round-trip cleanly through Simple Mode saves. **`mint_item_key`** pads slugs below 3 chars (the KeyValidator floor) so owner-friendly short codes like "U1" don't bounce. **REST surface** under `/configkit/v1/product-builder/*`: `/recipes`, `/auto-managed`, `/{id}/state`, `/{id}/product-type`, `/{id}/pricing`, `/{id}/fabrics`, `/{id}/profile-colors`, `/{id}/stangs`, `/{id}/motors`, `/{id}/controls`, `/{id}/accessories`, `/{id}/operation-mode`, `/{id}/checklist`, `/{id}/enable`. Capability gate: `configkit_manage_products` (same as the existing binding flow). **Shared JS helper** `ConfigKit.autoManaged` (admin.js) caches the registry snapshot so Module / Library / Lookup tables / Templates list rows render the 🔧 badge consistently. Every form save returns owner-friendly errors that never leak `template_key`, `library_key`, `module_key`. **Status: complete** (UI shipped in dalis 2 — see row above). The operation-mode rule auto-wiring (creating show/hide rules in the template when the owner picks "Both") is still deferred to a future chunk; the JS UI does the show/hide visually based on the recorded mode in the meantime so customers see the right options. **Tests**: 23 ProductBuilderServiceTest cases (recipe listing, family + template skeleton, idempotent re-set, pricing replace_all, fabric library replace, motor bundles with fixed_price, controls / colors libraries, operation mode validation, ready checklist false / true, enable refuses / writes meta). Suite 540 / 1429 → 563 / 1518; engine purity grep at zero. |
| Phase 4.2c — modules drive library / item / import / template universally | complete | Owner audit found that modules felt decorative — their declared capabilities and `attribute_schema_json` weren't reliably driving downstream forms. This phase replaces that with one rendering pipeline shared by every module type, no per-preset hardcoding. **CapabilityFormSchema** is the single PHP service every form reads from: `for_library_items()` returns universal + capability + inherited + attribute fields; `for_library()` returns the brand/collection fields that live on the library record; `import_columns()` returns the Excel header whitelist with aliases (`price_group_key` also accepts `price_group`; `woo_product_id` accepts `woo_product_sku`; attribute keys accept both bare and `attr.{key}`). Pure-PHP, no WP / DB. Attribute schema is normalised regardless of how it's stored — legacy `{key:type-string}` and the new rich `{key:{label,type,options,required,sort_order}}` both decode to the same output, with enum-without-options gracefully falling back to text. **AttributeSchemaService** validates + sanitises both shapes on the way through ModuleService::validate / ::sanitize so the module record always carries the rich shape after save. LibraryItemService::matches_type now accepts the rich types (text/number/enum) plus legacy aliases; enum attributes are also checked against their declared options at item-save time. **Module form** gains a structured Custom attributes editor (Label / auto-keyed snake_case Key / Type dropdown / Options when type=enum / Required / × remove / + Add attribute), replacing the old JSON-only Advanced view (kept as a power-user fallback inside Show advanced). Legacy schemas are normalised on entry so existing modules render without any migration step. **ModuleTypePresets** ship attribute_schema defaults — Textiles seeds fabric_code / material / transparency (enum) / blackout / flame_retardant / eco_label; Colors seeds pantone_code / hex_code; Motors seeds protocol (enum) / power_supply (enum) / torque_nm; Controls (NEW preset, allowed_field_kinds = [input, addon]) seeds channels / protocol; Accessories + Custom ship empty. apply_to_payload seeds attribute_schema only when the caller hasn't shipped one. **Library form** gains a "Module decides what fields appear here and on items in this library." intro and now redirects to the library detail page after save (capturing the new id from the POST response) so the owner lands somewhere they can immediately add items or trigger an import — not back at the list. **Library item form** renders the module's attribute schema in the rich shape: each attribute's display label is shown (not the snake_case key), required attrs get a `*` suffix, type drives the input — text → text input, number → number, boolean → checkbox, enum → dropdown of declared options. Brand and Collection render as read-only chips under a collapsed "Inherited from library" section when the module supports them. Helper text: "These fields come from the {Module Name} module." **LibraryItemParser** gains an optional module context (`set_module_context`) that lets the runner inject the target library's module before parse — schema-declared attribute headers route into `normalized.attributes` instead of `unknown_columns`. Headers `attr.fabric_code` rewrite to `fabric_code`; `price_group` and `active` canonicalise to `price_group_key` / `is_active`. **ModuleDetector** scores Excel headers against active modules using `matched / non-universal-total` (universal headers like library_key / item_key / label are excluded from the denominator so they don't inflate every score equally). pick_best returns the highest-scoring module above the threshold (default 60 %) plus a public ranked list. **QuickImportService.detect()** runs the detector for Format C files and returns `suggested_module` + a `module_match` block (matched / total / ratio / threshold / ranked) so the wizard can pre-fill the module dropdown and explain "Detected module: Textiles (87 % of headers match)". **Template field-edit "Source · libraries"** replaced its free-text CSV with a checklist that only shows libraries whose module's allowed_field_kinds includes the field's kind — input fields can't accidentally pick an addon-only Motors library, and the picker explains "Edit the module's 'Where this module can be used' checkboxes to tick {kind}" when nothing matches. New tests: 11 CapabilityFormSchemaTest, 9 AttributeSchemaServiceTest, 3 ModuleTypePresetsTest extensions, 3 LibraryItemParserTest extensions (attr.* alias, module-schema attribute routing with + without context, price_group canonicalisation), 6 ModuleDetectorTest, plus updated ModuleServiceTest cases for the rich-shape round-trip. Suite 508 / 1313 → 540 / 1429; engine purity grep at zero. |
| Phase 4 dalis 4 — real-data testing fixes                | complete | Owner ran a Sologtak data drill on demo.ovanap.dev/sol1/ and surfaced 5 friction bugs; this chunk fixed all five. **BUG 1** — migration `0016_create_import_rows.php` had `row_number` unquoted in CREATE TABLE columns AND in the `KEY batch_row` index. `row_number` is a reserved word in MySQL 8.0+ / MariaDB 10.2+ (window functions), so dbDelta() silently swallowed the syntax error and the table never got created — but Runner marked the migration applied because `up()` returned cleanly. Owner had to create the table by hand. Fix: backticked `row_number` everywhere, and added a two-layer Runner safety net — optional `Migration::expected_tables(\wpdb): list<string>` (Runner verifies post-up()) plus wpdb->last_error inspection (zeroed before up(), non-empty after fails the migration). Audited the other 16 migrations; only `status` and `level` came up, both non-reserved keywords that work fine. **BUG 2** — typing one letter into Name on Modules → New module ejected the cursor; same on Libraries / Lookup tables / Families / Templates / Step / Rule / Field-wizard. Cause: the onInput handler called render() inside the slugify-into-tech-key branch, rebuilding the form via root.replaceChildren() and wiping the input mid-keystroke. Fix: replace render() inside text-input handlers with a targeted DOM patch (read the technical-key input by id, assign the new value); the owner's input element stays in place; `userEdited{Key}` flag (per entity) flips when the owner edits the technical-key input directly so manual keys aren't clobbered, and resets on showNew*Form(). **BUG 3** — owner typing "Motioras vienas" got rejected for "library_key must be at least 3 characters" because the technical key was a visible required field on every entity create form. Fix: shared transliterating slugify in admin.js (`ConfigKit.slugify`) covers Lithuanian (ą č ę ė į š ų ū ž), Norwegian/Danish/Swedish (ø å æ ö ä ü), Polish (ł ń ó ś ź ż), German (ß), and common Latin diacritics; `fallbackPrefix` handles names that slugify below 3 chars so the owner never sees the min-length error ("M" → "module_m"). Technical-key fields move into a collapsed "Technical key (auto-generated)" fieldset on every create form; existing edit forms keep the field visible (collapsed: isNew). Verified: 'Motioras vienas' → 'motioras_vienas', 'Markisė į stogą' → 'markise_i_stoga', 'Pergolas for export clients' → 'pergolas_for_export_clients'. **BUG 4** — library item create / import surfaced "Library item could not be created. (validation_failed) [HTTP 400]" without saying which field failed. Fix: top-level error message now includes the first specific field's reason ("Library item could not be created — sku 'DCK-U171' already exists in this library. (+1 more)"); structured per-field errors stay in the response body. Added missing SKU-uniqueness check to LibraryItemService::validate (new `LibraryItemRepository::sku_exists_in_library`). **BUG 5** — Quick import (Excel-first wizard). New `src/Service/QuickImportService.php` plus `POST /imports/quick/detect` + `POST /imports/quick/create` collapse "Create entity → Save → Open → Import" into one upload: drop .xlsx → server detects format (A/B → lookup table, C → library), proposes a name from the filename and a slug, owner confirms (Name auto-keys the slug; Module dropdown for libraries), one round-trip creates the entity AND runs the parse + commit pipeline. Idempotent against existing keys (re-runs route into the existing table/library instead of erroring). Multi-library Format C files are rejected with a pointer back to the standard wizard. Per-type capability gate matches the standard wizard. New top-of-page UI on ConfigKit → Imports with drag-drop, confirm step (Technical key collapsed, like every other create form), and result page linking to the new entity. New tests: 2 RunnerTest cases (silent dbDelta failure / wpdb->last_error trip), 1 LibraryItemServiceTest (duplicate SKU), 1 LibraryItemsControllerPreviewTest (specific-field message), 8 QuickImportServiceTest (detect grid + long C, multi-lib reject, garbage reject, create+import for both targets, library-without-module reject, idempotent re-run). Suite 496 / 1272 → 508 / 1313; engine purity grep at zero. |
| Library items import                                     | complete | Phase 4 dalis 3. FormatDetector grows a Format C signature (header row containing `library_key` + `item_key` + `label`; library-items detection wins over lookup-cells when both signatures appear). LibraryItemParser is a standalone Format C reader: every documented column normalised into a typed shape, with module-capability optionals (brand, collection, color_family, image_url, main_image_url, filter_tags, compatibility_tags) split into an `attributes` bucket and unknown owner-supplied headers tracked in `normalized.unknown_columns`. LibraryItemValidator covers IMPORT_WIZARD_SPEC §6 in full: required keys, `^[a-z][a-z0-9_]{2,63}$` shape on item_key, library existence + wizard-target match, numeric ranges, `price_source` / `item_type` enum allow-lists, `woo_product_sku → woo_product_id` resolution via a new `WooSkuResolver` adapter (interface + production `WooSkuResolverImpl` in src/Adapters/), capability-mismatch warnings that drop the offending column from `normalized` so the runner doesn't persist data the module can't surface, and duplicate (library_key, item_key) within file → last row wins. LibraryItemRunner mirrors the lookup-cells lifecycle (received → parsing → parsed → validated → committing → applied) with idempotent insert/update by (library_key, item_key) and replace_all that soft-deletes every active item in the library before inserting (`LibraryItemRepository::soft_delete_all_in_library`). LibraryItemRepository gains `find_by_library_and_key()` so the runner avoids a second round-trip per row. ImportService now dispatches by `import_type`: lookup_cells stays on the existing Runner, library_items routes to the new LibraryItemRunner; the lookup-cells path is unchanged. ImportsController accepts `import_type=library_items` with `target_library_key` and gates per-type capabilities (libraries vs lookup-tables). Wizard JS: Step 1 destination radio offers Lookup table cells / Library items with the right target dropdown; URL params `target_type=library_items&target_library_key=KEY` (or `&target_id=N`) skip step 1 contextually; Step 3 preview adapts to library-item columns (item_key, label, SKU, price) and shows columns parsed + price sources + item types; Step 4 reads "X items imported into 'Library Name'" with an Open Library link. Library detail page header gains an `⬆ Import from Excel` button mirroring the lookup-tables analog from Phase 4 polish. 24 new tests across LibraryItemParserTest (6), LibraryItemValidatorTest (11), LibraryItemRunnerTest (7); StubLibraryItemRepository now overrides find_by_library_and_key + soft_delete_all_in_library so runner tests don't reach the typed-property `$wpdb`. Suite 472 / 1206 → 496 / 1272 green; engine purity grep zero matches; lookup_cells path remains green throughout. |
| Diagnostics catalogue expansion (warnings + info)        | pending  |                                                                        |
| Product Readiness Board                                  | pending  |                                                                        |

**Engine purity preserved.** `grep -rE "wp_\|get_option\|WP_Query\|\\$wpdb" src/Engines/`
returns zero matches. Phase 2 + 3 + 3.5 + 3.6 + 4 + 4.2b + 4.2c +
4.3 PHPUnit tests green (565 / 1531).
...
**Phase 4 dalis 3** then landed library items as a second import
target. FormatDetector grows Format C; LibraryItemParser /
LibraryItemValidator / LibraryItemRunner mirror the lookup-cells
trio with library-items shape. WooSkuResolver adapter lets the
file ship `woo_product_sku` instead of `woo_product_id` so owners
working in their existing Woo SKU vocabulary don't have to look up
post ids. Wizard step 1 grows a destination radio (lookup table
cells / library items) and the library detail page gets the same
⬆ Import from Excel header button the lookup tables page already
had. 24 new PHPUnit tests; suite 472 / 1206 → 496 / 1272.

**Phase 4.3** — Product Builder Simple Mode orchestrator. Owners
think in products ("I have a Markise"), not in modules → libraries
→ items → templates → fields → rules → bindings. The orchestrator
collapses that chain: pick a product type → blocks appear (pricing,
fabrics, motors, etc.) → each block save provisions / updates the
underlying entities silently with auto-generated technical keys.
ProductTypeRecipes ships five types (Markise, Screen, Pergola,
Terrassetak, Custom). AutoManagedRegistry tags every entity Simple
Mode creates so Advanced admin lists render a 🔧 Auto-managed
badge alongside the technical key.

**Phase 4.3 dalis 2** then landed the UI on top — a single-page
Product Builder mounted in the Woo product ConfigKit tab, with 10
blocks rendered from the recipe and one round-trip via
`/snapshot` to populate everything. Each block hits the
orchestrator endpoints (`/pricing`, `/fabrics`,
`/profile-colors`, `/operation-mode`, `/stangs`, `/motors`,
`/controls`, `/accessories`, `/enable`) and writes friendly
status updates to the header pill (Disabled → Setup in progress
→ Ready → Live). Excel imports for pricing + fabrics are wired
through the existing `/imports` wizard with auto-managed target
keys. Owner toggles to the existing 8-section advanced view via
a header button when they need power-user controls. Suite 563 /
1518 → 565 / 1531.

**Phase 4.2c** — owner-driven audit found that modules felt
decorative; their declared capabilities didn't reliably drive the
library item form, Excel import column mapping, or template source
picker. Fixed universally: one CapabilityFormSchema service every
form reads from, one AttributeSchemaService that validates +
sanitises both legacy and rich schema shapes, structured Custom
attributes editor on the Module form, library item form renders
attributes with the rich type system (text/number/boolean/enum
with declared options), parser opts into a module context so
schema attributes route into the attributes bucket instead of
unknown_columns, ModuleDetector scores Excel headers against
active modules so the Quick import wizard pre-fills the module
dropdown when the file's columns clearly belong to one, and the
template field-edit source picker is now a checklist filtered by
allowed_field_kinds. Owner can build a custom module with any
capability mix and the entire admin reacts without code changes.
32 new PHPUnit tests; suite 508 / 1313 → 540 / 1429.

**Phase 4 dalis 4** — five fixes uncovered by an owner-led
real-data drill on demo.ovanap.dev/sol1/. Migration 0016 had an
unquoted `row_number` reserved word; backticked it and added a
two-layer Runner safety net (optional expected_tables() check +
wpdb->last_error inspection). Cursor-jump on every entity Name
field replaced a render()-on-every-keystroke pattern with a
targeted DOM patch; owner can type "Markise" without losing the
input. Technical-key fields move into a collapsed
"Technical key (auto-generated)" disclosure on every create form
backed by a shared ConfigKit.slugify that transliterates Lithuanian
/ Norwegian / Polish / German diacritics — "Motioras vienas" →
"motioras_vienas", "Markisė į stogą" → "markise_i_stoga", "M" gets
a fallback prefix so the 3-char minimum never trips. Library item
create / import errors now surface the specific field that failed
("sku 'DCK-U171' already exists in this library.") instead of a
generic 400; SKU uniqueness check added. Quick import (Excel-first
wizard) — drop .xlsx, server detects format (A/B → lookup table,
C → library) and proposes a name from the filename, owner confirms,
one round-trip creates the entity AND commits the import; idempotent
against existing keys. Multi-library Format C files are rejected
