...
final class LookupTablesController extends AbstractController {

	private const CAP = 'configkit_manage_lookup_tables';

	public function __construct( private LookupTableService $service ) {}
	...
	public function read( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		$record = $this->service->get_with_stats( (int) $request['id'] );
		if ( $record === null ) {
	...
	public function create( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		...
		if ( ! ( $result['ok'] ?? false ) ) {
			return $this->error( 'validation_failed', 'Lookup table could not be created.', [ 'errors' => $result['errors'] ?? [] ], 400 );
		}
	...
	public function update( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		...
		if ( ! ( $result['ok'] ?? false ) ) {
			...
			return $this->error( 'validation_failed', 'Lookup table could not be updated.', [ 'errors' => $result['errors'] ?? [] ], 400 );
		}
	...
	public function soft_delete( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		...
		if ( ! ( $result['ok'] ?? false ) ) {
			...
			return $this->error( 'delete_failed', 'Lookup table could not be deleted.', [ 'errors' => $result['errors'] ?? [] ], 400 );
		}
