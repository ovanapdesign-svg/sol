...
final class FieldsController extends AbstractController {
	...
	public function register_routes(): void {
		\register_rest_route(
			self::NAMESPACE,
			'/templates/(?P<id>\d+)/steps/(?P<step_id>\d+)/fields',
			[
				[
					'methods'             => 'GET',
					'callback'            => [ $this, 'list' ],
		...
		\register_rest_route(
			self::NAMESPACE,
			'/templates/(?P<id>\d+)/steps/(?P<step_id>\d+)/fields/reorder',
			[
				[
					'methods'             => 'POST',
		...
		\register_rest_route(
			self::NAMESPACE,
			'/templates/(?P<id>\d+)/fields/(?P<field_id>\d+)',
			[
				[
					'methods'             => 'GET',
					'callback'            => [ $this, 'read' ],
	...
	public function list( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		$result = $this->service->list_in_step( (int) $request['id'], (int) $request['step_id'] );
		if ( $result === null ) {
			return $this->error( 'not_found', 'Template or step not found.', [], 404 );
		}
		return $this->ok( $result );
	}

	public function read( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		$record = $this->service->get( (int) $request['id'], (int) $request['field_id'] );
		if ( $record === null ) {
	...
	public function create( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		$result = $this->service->create( (int) $request['id'], (int) $request['step_id'], $this->payload( $request ) );
		if ( ! ( $result['ok'] ?? false ) ) {
			$first = $result['errors'][0]['code'] ?? '';
			if ( $first === 'not_found' ) {
				return $this->error( 'not_found', 'Template or step not found.', [], 404 );
			}
			return $this->error( 'validation_failed', 'Field could not be created.', [ 'errors' => $result['errors'] ?? [] ], 400 );
		}
	...
	public function update( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		...
		$result = $this->service->update( (int) $request['id'], (int) $request['field_id'], $payload, $expected );
		if ( ! ( $result['ok'] ?? false ) ) {
			$first = $result['errors'][0]['code'] ?? '';
			if ( $first === 'conflict' ) {
			...
			return $this->error( 'validation_failed', 'Field could not be updated.', [ 'errors' => $result['errors'] ?? [] ], 400 );
		}
		return $this->ok( [ 'record' => $result['record'] ?? null ] );
	}

	public function delete( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		$result = $this->service->delete( (int) $request['id'], (int) $request['field_id'] );
		if ( ! ( $result['ok'] ?? false ) ) {
			$first = $result['errors'][0]['code'] ?? '';
			if ( in_array( $first, [ 'not_found', 'template_not_found' ], true ) ) {
				return $this->error( 'not_found', $result['errors'][0]['message'] ?? 'Not found.', [], 404 );
			}
			return $this->error( 'delete_failed', 'Field could not be deleted.', [], 400 );
		}
		return $this->ok( [ 'deleted' => true, 'id' => (int) $request['field_id'] ] );
	}
