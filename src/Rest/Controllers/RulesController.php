...
final class RulesController extends AbstractController {
	...
	private const CAP = 'configkit_manage_rules';
	...
	public function register_routes(): void {
		...
		\register_rest_route(
			self::NAMESPACE,
			'/templates/(?P<id>\d+)/rules/reorder',
			[
				[
					'methods'             => 'POST',
		...
		\register_rest_route(
			self::NAMESPACE,
			'/templates/(?P<id>\d+)/rules/(?P<rule_id>\d+)',
			[
				[
					'methods'             => 'GET',
					'callback'            => [ $this, 'read' ],
	...
	public function read( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		$record = $this->service->get( (int) $request['id'], (int) $request['rule_id'] );
		if ( $record === null ) {
			return $this->error( 'not_found', 'Rule not found.', [], 404 );
		}
	...
	public function create( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		...
		if ( ! ( $result['ok'] ?? false ) ) {
			...
			return $this->error( 'validation_failed', 'Rule could not be created.', [ 'errors' => $result['errors'] ?? [] ], 400 );
		}
	...
	public function update( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		...
		$result = $this->service->update( (int) $request['id'], (int) $request['rule_id'], $payload, $expected );
		if ( ! ( $result['ok'] ?? false ) ) {
			$first = $result['errors'][0]['code'] ?? '';
			if ( $first === 'conflict' ) {
			...
			return $this->error( 'validation_failed', 'Rule could not be updated.', [ 'errors' => $result['errors'] ?? [] ], 400 );
		}
		return $this->ok( [ 'record' => $result['record'] ?? null ] );
	}

	public function soft_delete( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		$result = $this->service->soft_delete( (int) $request['id'], (int) $request['rule_id'] );
		if ( ! ( $result['ok'] ?? false ) ) {
			$first = $result['errors'][0]['code'] ?? '';
			if ( in_array( $first, [ 'not_found', 'template_not_found' ], true ) ) {
				return $this->error( 'not_found', $result['errors'][0]['message'] ?? 'Not found.', [], 404 );
			}
			return $this->error( 'delete_failed', 'Rule could not be deleted.', [], 400 );
		}
		return $this->ok( [ 'deleted' => true, 'id' => (int) $request['rule_id'] ] );
	}
