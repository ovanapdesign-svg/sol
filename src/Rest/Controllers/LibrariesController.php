...
final class LibrariesController extends AbstractController {
	...
	public function list( \WP_REST_Request $request ): \WP_REST_Response {
		$filters = [];
		if ( $request->get_param( 'module_key' ) !== null && $request->get_param( 'module_key' ) !== '' ) {
			$filters['module_key'] = (string) $request->get_param( 'module_key' );
		}
		...
		$result   = $this->service->list( $filters, $page === 0 ? 1 : $page, $per_page === 0 ? 100 : $per_page );
	...
	public function update( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		...
		if ( ! ( $result['ok'] ?? false ) ) {
			...
			return $this->error( 'validation_failed', 'Library could not be updated.', [ 'errors' => $result['errors'] ?? [] ], 400 );
		}
	...
	public function soft_delete( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
		...
		if ( ! ( $result['ok'] ?? false ) ) {
			...
			return $this->error( 'delete_failed', 'Library could not be deleted.', [ 'errors' => $result['errors'] ?? [] ], 400 );
		}
