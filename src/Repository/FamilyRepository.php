...
class FamilyRepository {
	...
	public const TABLE = 'configkit_families';
	...
	public function find_by_key( string $key ): ?array {
		...
		/** @var array<string,mixed>|null $row */
		$row = $this->wpdb->get_row(
			$this->wpdb->prepare( "SELECT * FROM `{$table}` WHERE family_key = %s", $key ),
			ARRAY_A
		);
		return is_array( $row ) ? $this->hydrate( $row ) : null;
	}
	...
	public function key_exists( string $key, ?int $exclude_id = null ): bool {
		...
		if ( $exclude_id === null ) {
			$value = $this->wpdb->get_var(
				$this->wpdb->prepare( "SELECT id FROM `{$table}` WHERE family_key = %s LIMIT 1", $key )
			);
		} else {
			$value = $this->wpdb->get_var(
				$this->wpdb->prepare(
					"SELECT id FROM `{$table}` WHERE family_key = %s AND id <> %d LIMIT 1",
					$key,
					$exclude_id
				)
			);
	...
	public function create( array $data ): int {
		...
		if ( $ok === false || $ok === 0 ) {
			throw new \RuntimeException( 'Failed to insert family: ' . (string) $this->wpdb->last_error );
		}
	...
	public function update( int $id, array $data ): void {
		...
		if ( $result === false ) {
			throw new \RuntimeException( 'Failed to update family: ' . (string) $this->wpdb->last_error );
		}
	}
	...
	public function soft_delete( int $id ): void {
		...
		if ( $result === false ) {
			throw new \RuntimeException( 'Failed to soft delete family: ' . (string) $this->wpdb->last_error );
		}
	}
	...
	private function dehydrate( array $data ): array {
		return [
			'family_key'  => (string) ( $data['family_key'] ?? '' ),
